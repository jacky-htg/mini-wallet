...
func Response(w http.ResponseWriter, data interface{}, status string, httpCode int) error {

	// Convert the response value to JSON.
	res, err := json.Marshal(ResponseFormat{Status: status, Data: data})
	if err != nil {
		return err
	}
...
func ResponseOK(w http.ResponseWriter, data interface{}, HTTPStatus int) error {
	return Response(w, data, StatusMessageOK, HTTPStatus)
}
...
func ResponseError(w http.ResponseWriter, err error) error {
	...
	if webErr, ok := err.(*Error); ok {
		if err := Response(w, nil, webErr.MessageStatus, webErr.HTTPStatus); err != nil {
			return err
		}
		return nil
	}
	...
	if err := Response(w, nil, StatusMessageInternalServerError, http.StatusInternalServerError); err != nil {
		return err
	}
	return nil
}
